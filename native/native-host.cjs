...
function sanitizeProfile(profile = {}) {
  ...
  return {
    host: String(profile.host || '').trim(), port: Math.max(1, Math.min(65535, Number(profile.port || 22))),
    username: String(profile.username || '').trim(), authMethod,
...
function registerInstance(extra = {}) {
  ...
  atomicJson(instanceFile(), {
    product: '986Code Bridge', version: VERSION, hostName: HOST_NAME,
    instanceId: instance.instanceId, label: instance.label,
    extensionVersion: instance.extensionVersion || null,
    permissions: instance.permissions,
    pid: process.pid, host: '127.0.0.1', port: address.port,
    token: localToken, startedAt: instance.startedAt,
...
function waitForExtension(command, timeoutMs = 30000) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => { pendingHttp.delete(requestId); reject(new Error('Extension command timeout')); }, timeoutMs);
...
function runProcess(file, args, timeoutMs = 30000) {
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      clearTimeout(timer);
      resolve({ ok: code === 0 && !timedOut, code, timedOut, stdout: stdout.slice(0, cap), stderr: stderr.slice(0, cap) });
    });
  });
}

async function executeSsh(command = {}) {
  if (instance?.permissions?.power !== true) throw new Error('POWER tier is disabled for this browser instance.');
...
async function handleHttp(req, res) {
  ...
  if (req.method === 'GET' && url.pathname === '/v1/info') {
    return sendJson(res, 200, {
      ok:true, instanceId:instance?.instanceId, label:instance?.label,
      permissions:instance?.permissions, extensionVersion:instance?.extensionVersion,
      nativeVersion:VERSION, pid:process.pid
    });
  }
  if (req.method === 'POST' && url.pathname === '/v1/command') {
    try {
      ...
      const result = target === 'ssh' ? await executeSsh(command) : await waitForExtension(command, Number(command.timeoutMs || 30000));
      return sendJson(res, result?.ok === false ? 422 : 200, result);
    } catch (error) {
...
async function startHttp() {
  ...
  httpServer = http.createServer((req, res) => { handleHttp(req, res).catch((e) => sendJson(res, 500, { ok:false, error:e.message })); });
...
async function handleNativeRequest(message) {
  ...
  if (action === 'ssh.exec') return await executeSsh(message.command || {});
  if (action === 'native.info') {
    const address = httpServer?.address();
    return {
      ok:true, version:VERSION, hostName:HOST_NAME,
      instanceId:instance?.instanceId, label:instance?.label,
      permissions:instance?.permissions,
...
async function handleNative(message = {}) {
  if (message.type === 'hello') {
    instance = {
      ...
      permissions:{ read:message.permissions?.read !== false, write:message.permissions?.write !== false, power:message.permissions?.power === true },
    ...
    registerInstance({ status:'connected' });
    const address = httpServer.address();
    writeNative({ type:'hello.ack', ok:true, nativeVersion:VERSION, instanceId:instance.instanceId, label:instance.label, controlPlane:{host:'127.0.0.1',port:address.port} });
    return;
  }
  if (message.type === 'policy.update') {
    ...
    registerInstance({ status:'connected' });
    writeNative({ type:'policy.ack', ok:true, requestId:message.requestId || null, label:instance.label, permissions:instance.permissions });
    return;
  }
  ...
  if (message.type === 'native.request') {
    ...
    try {
      ...
      writeNative({ type:'native.response', requestId, result:{ ok:false, error:String(error?.message || error) } });
    }
    return;
  }
  if (message.type === 'ping') {
    writeNative({ type:'pong', at:new Date().toISOString(), version:VERSION });
  }
}
...
function consumeNativeInput(chunk) {
  ...
  while (inputBuffer.length >= 4) {
    ...
    Promise.resolve(handleNative(message)).catch((error) => writeNative({ type:'error', ok:false, error:String(error?.message || error) }));
  }
}
function cleanup() {
