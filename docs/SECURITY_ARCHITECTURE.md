...
```text
User ChatGPT / AI account or local MCP client
        |
User browser profile + session
        |
986Code Bridge extension
        | Native Messaging
986Code native control plane (127.0.0.1 only)
        |
CLI / stdio MCP adapter / SSH client
        |
...
5. SSH profile metadata is owned by the native user-side layer; extension storage must not hold SSH passwords, passphrases, private-key material or bearer tokens.
6. Local audit history is metadata-redacted by default and never stores typed values or SSH command bodies.
7. Each native-host instance generates its own local bearer token and binds only to `127.0.0.1`; callers must select an explicit browser instance.
8. Future direct API integrations must use per-user authorization or BYOK; a maintainer-owned shared key is prohibited.
